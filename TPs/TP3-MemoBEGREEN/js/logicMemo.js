...
function MemoTest(tipoJuego, dificultad, nombreGanador) {
    var paresEncontrados = 0;
    var intentos = 0;
   // var dificultad = 1;
    var limiteIntentos = dificultad;
    move(paresEncontrados);
     ...
     */
    function finPartida(gano, nombreGanador, tipoJuego, dificultad) {
            ...
            .then((value) => {
                switch (value) {
                    ...
                    case "si":
                        intentos = 0;
                        paresEncontrados = 0;
                        limiteIntentos = dificultad;
                        MemoTest(tipoJuego, dificultad);
                        break;
                        /*---------------Record!!----------------*/
                    case "record":
                        startRanking();
                        if (gano) {
                            ingresarGanador(nombreGanador, tipoJuego, intentos);
                            break;
                        } else {
    ...
    function guardarIDsSegundaCarta(id) {
        if (intentos == limiteIntentos) {
            finPartida(false, nombreGanador, tipoJuego, dificultad);
        } else {
        ...
        if (primerImagen.imagen == null) { //si no tengo nada guardado como 1era imagen,
            ...
            if (primerImagen.imagen == arrayJuego[id] && //Si el src guardada en primerImagen es igual al numero de index
                ...
                if (paresEncontrados == arrayJuego.length / 2) { //si se encontraron todos los pares, termina partida
                    finPartida(true, nombreGanador, tipoJuego, dificultad);
    ...
    /*---progreso!---*/
    function move(paresEncontrados) {
        var elem = document.getElementById("barra");
        var width = paresEncontrados * 16.7;
